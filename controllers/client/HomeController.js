const { Ticket, SoldTicket, Order, OrderDetail, sequelize } = require( '../../database/models/index' )
...
class Controller
{
    ...
    async pay ( req, res )
    {
        var order = await Order.findOne( { where: { id: req.body?.id }, include: OrderDetail } )
        ...
        try
        {
            ...
            for ( const e of order?.OrderDetails )
            {
                ...
                {
                    soldTicketArr.push( await SoldTicket.create( {
